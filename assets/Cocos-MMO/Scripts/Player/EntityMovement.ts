...
export class EntityMovement extends Component {
  ...
  private handleInput(deltaTime: number) {
    ...
    if (Input.getKey(macro.KEY.q) || Input.getKey(macro.KEY.e)) {
      ...
      this._rotationAngle =
        (Input.getKey(macro.KEY.q) ? 1 : -1) * this.movementRotationSpeed;
      this.node.rotate(
        Quat.fromEuler(
          this._tempQuat,
          0,
          this._rotationAngle * this.rotateSpeed,
          0
        )
      );
    } else {
    ...
    if (
      Input.getKey(macro.KEY.a) ||
      Input.getKey(macro.KEY.d) ||
      Input.getKey(macro.KEY.w) ||
      Input.getKey(macro.KEY.s)
    ) {
      let lateralVec: Vec3 =
        this.cameraNode != null
          ? MathHelpers.nodeRight(this.cameraNode)
          : MathHelpers.nodeRight(this.node);
      lateralVec.y = 0;

      let forwardVec: Vec3 =
        this.cameraNode != null
          ? this.cameraNode.forward.negative()
          : this.node.forward.negative();
      forwardVec.y = 0;
      ...
      if (!this._isForcingRotation) {
        ...
        this.node.setRotation(
          Quat.lerp(
            ...
            this.node.rotation,
            desRot,
            this.movementRotationSpeed * deltaTime
          )
        );
      }

      let verticalMov: number = Input.getKey(macro.KEY.w)
        ? 1
        : Input.getKey(macro.KEY.s)
        ? -1
        : 0;
      let horizontalMov: number = Input.getKey(macro.KEY.a)
        ? -1
        : Input.getKey(macro.KEY.d)
        ? 1
        : 0;

      this._moveVec = forwardVec
        .multiplyScalar(verticalMov * deltaTime)
        .add(lateralVec.multiplyScalar(horizontalMov * deltaTime))
        .multiplyScalar(this._isSprinting ? this.SprintSpeed : this.moveSpeed);
    ...
    if (this._currentInteractable && this._currentInteractable.isValid) {
      if (
        Input.getKeyDown(
          Input.getInputKeyValue(this._currentInteractable.interactionKey)
        )
      ) {
