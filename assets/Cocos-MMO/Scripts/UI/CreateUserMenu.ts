...
export class CreateUserMenu extends Component {
  ...
  public consumeSeatReservationHandler: (
   ...
   */
  public async onButtonEvent_SubmitSignUp() {
    this.signUpErrorMsg.string = "";

    this._attemptingSignUp = true;
    this.signUpSpinner.active = true;

    const res = await MMOManager.Instance.userSignUp(
      this.usernameInput.string,
      this.emailInput.string,
      this.passwordInput.string
    );
  ...
  public onButtonEvent_SubmitLogIn() {
    ...
    MMOPlayerPrefs.email = this.rememberMe.isChecked
      ...
      : "";
    MMOPlayerPrefs.password = this.rememberMe.isChecked
