...
export class LobbyController extends Component {
  ...
  public async onButtonEvent_Begin() {
    ...
    if (MMOPlayerPrefs.accountExists) {
      try {
        await this.createUserMenu.logUserIn(
          MMOPlayerPrefs.email,
          MMOPlayerPrefs.password
        );
      } catch (error: any) {
        ...
        this.createUserMenu.updateErrorText(error.message);
