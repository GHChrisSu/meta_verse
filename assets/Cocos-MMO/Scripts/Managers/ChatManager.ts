...
export class ChatManager extends Component {
  ...
  public onQueueAdded: EventDispatcher<string> = new EventDispatcher<string>(
    "onQueueAdded"
  );
  public onQueueRemoved: EventDispatcher<string> = new EventDispatcher<string>(
    "onQueueRemoved"
  );
  ...
  private registerForMessages() {
    if (this._chatRoom) {
      ...
      this._chatRoom.state.chatQueue.onRemove = (
        item: ChatQueue,
        key: string
      ) => {
  ...
  private connectIDs() {
    ...
    if (entity && this._chatRoom) {
      ...
      console.error(
        `Chat Manager - ConnectIDs - Either No Entity or No Chat Room Yet!`
      );
    }
  }
