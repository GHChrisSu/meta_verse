...
import { DDZ_ROOM_STATE } from "../../../Server/src/game/defines";
...
import Connector from "../../Cocos-MMO/Scripts/Helpers/Connector";
import { EventDispatcher } from "../../Cocos-MMO/Scripts/Helpers/EventDispatcher";
...
export default class GameScene extends Component {
  ...
  public ddzRoom: Colyseus.Room<DDZRoomState>;

  private onRoomStateChange(state: DDZRoomState) {
    log(state);

    log(Connector.client.auth);
  ...
  async onLoad() {
    this.ddzRoom = await Connector.client.joinOrCreate<DDZRoomState>(
      "ddz_room",
      {
        roomID: "xxx",
      }
    ...
    this.roomstate = DDZ_ROOM_STATE.ROOM_INVALID;
